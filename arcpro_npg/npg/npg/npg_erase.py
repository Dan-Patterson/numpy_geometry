...
def _wn_clip_(pnts, poly, all_info=True):
    ...
    def _w_(a, b, all_info):
        ...
        x0_x2 = x0 - x2[:, None]
        y0_y2 = y0 - y2[:, None]
        # a_num = a_0 - a_1 : a_0 = y0_y2 * x3_x2  a_1 = y3_y2 * x0_x2
        # b_num = b_0 - b_1 : b_0 = y0_y2 * x1_x0  b_1 = y1_y0 * x0_x2
        a_num = y0_y2 * x3_x2 - y3_y2 * x0_x2
        b_num = y0_y2 * x1_x0 - y1_y0 * x0_x2
        #
        # pnts in poly
    ...
    vals = [x_pnts, p_in_c, c_in_p, x_type, wn_, whr]
    # if ..outs needed [x_pnts, p_in_c, c_in_p, c_out_p, ... whr]
    if all_info:
        return vals
    return wn_
...
def _nodes_(p_in_c, c_in_p, clp, poly, x_pnts):
    ...
    cp_eq = np.array([c_eq_p, p_eq_c]).T
    cx_eq = np.array([c_eq_x, x_eq_c]).T
    px_eq = np.array([p_eq_x, x_eq_p]).T
    if (p_eq_c.size > 0) or (p_eq_x.size > 0):  # p_in_c + (c_eq_p, p_eq_x)
        p_in_c = np.unique(np.concatenate((p_in_c, p_eq_c, p_eq_x)))
    u = np.arange(poly.shape[0])
    p_out_c = u[np.in1d(u, p_in_c, assume_unique=True, invert=True)]
    p_out_c = p_out_c.tolist()
    # -- (2) clp, poly point equal
    if (c_eq_p.size > 0) or (c_eq_x.size > 0):  # c_in_p + (p_eq_c, c_eq_x)
        c_in_p = np.unique(np.concatenate((c_in_p, c_eq_p, c_eq_x)))
        # u = np.arange(clp.shape[0])
        # c_out_p = u[np.in1d(u, c_in_p, assume_unique=True, invert=True)]
    p_in_c = p_in_c.tolist()
    c_in_p = c_in_p.tolist()
    c_eq_p = sorted(list(set(c_eq_p.tolist())))
    c_eq_x = sorted(list(set(c_eq_x.tolist())))
    p_eq_c = sorted(list(set(p_eq_c.tolist())))
    p_eq_x = sorted(list(set(p_eq_x.tolist())))
    args = (p_in_c, c_in_p, c_eq_p, c_eq_x, p_eq_c, p_eq_x, cp_eq,
            cx_eq, px_eq)  # removed p_out_c from 2nd position
    return args


def _x_mkr_(to_chk, x_pnts, poly, p_in_c, c_in_p):  # p_eq_x, c_eq_x):
    ...
    # z2 = np.isin(crossings[:, :2], c_eq_x)
    # z3 = np.isin(crossings[:, 2:4], p_eq_x)
    # z0z2 = z0.astype('int') + z1.astype('int')
    # z1z3 = z2.astype('int') + z3.astype('int')
    #
    in_chks = np.concatenate((z0, z1), axis=1)
    xCheck = np.concatenate((crossings, in_chks.astype(int)), axis=1)
    # in_chks2 = np.concatenate((z0z2, z1z3), axis=1)
    # xCheck2 = np.concatenate((crossings, in_chks2), axis=1)
    # cr_in_hdr = "c0 c1 p0 p1 c0in c1in p0in p1in"
    #
    # -- intersection points
    ...
    if x0x1[-1].shape == (1, 2):
        ...
        x0x1[-1] = pad
    # x0x1_4 = np.asarray(x0x1).reshape(-1, 4)
    return xCheck, x0x1  # xCheck2
...
def split_seq(seq, prn=False):
    """Return a sequence split at its numeric gaps.  The defined gap is 1.
    ...
    if seq.ndim > 1 or seq.shape[0] <= 1:
        ...
        return [seq.tolist()]
    whr = np.nonzero((seq[1:] - seq[:-1]) != 1)[0]
    if whr.size > 0:
...
def p_type(ply, eqX, eqOther, inOther):
    ...
    v = [0, 1, 2, 3, 4, 5, 6, 7]
    d = dict(zip(k, v))
    z = np.zeros((ply.shape[0], 4), 'int')
    z[:, 0][eqX] = 1
    z[:, 1][eqOther] = 1
    z[:, 2][inOther] = 1
    keys = z[:, 0] * 100 + z[:, 1] * 10 + z[:, 2]
...
def erase(poly, clp):
    ...
    Sample data::
        ...
        t01 = np.array([[1., 0], [1., 2], [3., 2.], [8., 2], [8., 0.],
                        [0., 0]])
    ...
    vals = _wn_clip_(poly, clp, all_info=True)  # ** reversed
    x_pnts, pInc, cInp, x_type, wn_, whr = vals
    #
    if len(x_pnts) == 0:
    ...
    args = _nodes_(pInc, cInp, clp, poly, x_pnts)
    p_in_c, c_in_p, c_eq_p, c_eq_x, p_eq_c, p_eq_x = args[:6]
    cp_eq, cx_eq, px_eq = args[-3:]
    #
    p_out_c = list(set(np.arange(poly.shape[0])).difference(p_in_c))
    c_out_p = list(set(np.arange(clp.shape[0])).difference(c_in_p))
    #
    nC = clp.shape[0]
    nP = poly.shape[0]
    # -- run `_x_mkr_` for both types of analysis until I choose.
    # # xChk2 = p_eq_x, c_eq_x) removed from below
    xChk, x0x1 = _x_mkr_(whr_on, x_pnts_on, poly, p_in_c, c_in_p)
    # difference in index pairs for clp and poly
    dC_dP = np.abs(np.vstack((xChk[:, 1] - xChk[:, 0],
                              xChk[:, 3] - xChk[:, 2]))).T
    if isinstance(c_in_p, np.ndarray):  # to speed up inclusions tests
        c_in_p = c_in_p.tolist()        # arr.tolist() is faster than list(arr)
    if isinstance(p_in_c, np.ndarray):
        p_in_c = p_in_c.tolist()
    #
    c_prev, p_prev = [-1, -1]
    ...
    #
    p_seq = split_seq(p_out_c)
    c_seq = split_seq(c_in_p)
    Np_seq = len(p_seq)
    Nc_seq = len(c_seq)
    for cnt, xC in enumerate(xChk):  # xChk is the crossings list
        ...
        frum = p0_fr                 # used to reassign p0_fr on end check
        c0c1_ = in_case[:3]
        p0p1_ = in_case[4:]
        sub_ = []
        #
        seq_p = p_seq[cnt] if cnt <= Np_seq else []
        if len(seq_p) == 1 and 0 in seq_p and Np_seq > 1:
            seq_p = p_seq[cnt + 1]
        seq_c = c_seq[cnt] if cnt <= Nc_seq else []
        #
        if in_case == '0 1 1 0':  # b1,c0 c0,b1 b5,c1 c1,b5
            if p0_fr in seq_p or p0_to in seq_p:
                to_add = [[x0], poly[seq_p]]
            elif p1_fr in seq_p:  # p0_to != p1_fr
                to_add = [[x0], poly[seq_p]]
            for i in to_add:
                sub_.extend(i)
            if c1_fr in seq_c:
                if c0_fr <= seq_c[0]:
                    seq_c = seq_c[::-1]
                sub_.append(x1)
                sub_.extend(clp[seq_c])
            else:
                sub_.append(x1)
            p_seen.extend(seq_p)
            c_seen.extend(seq_c)
        #
        elif in_case == '1 0 0 1':
            if p0_fr in seq_p or p0_to in seq_p:
                to_add = [poly[seq_p], [x0]]
            for i in to_add:
                sub_.extend(i)
            if c0_fr in seq_c:
                if c0_fr >= seq_c[0]:
                ...
                sub_.extend(clp[seq_c])
                sub_.append(x1)
        # note :  should also check for seq_c since it may contain points
        if cnt + 1 == len(xChk) and len(p_seq) > cnt + 1:
            sub_.extend(poly[p_seq[cnt + 1]])
        # if len(p_seq) > 1:
        #     sub_.extend([poly[i] for i in p_seq if p1_to in i][0])
        # else:
        ...
        # -- subsequent crossings
            ...
            if p0_fr - p_prev > 1 and p_prev not in p_eq_c:
                if (p_prev in p_out_c) and (p_prev not in p_seen):
                    sub_.extend([poly[p_prev]])
                    p_seen.extend([p_prev])
                bf_p = _btw_chk_(p0_fr, p_prev, p_out_c, p_seen)
        ...
        if diff_C != 0:
            ...
            if btw_c and c1_fr - c0_fr == 1:  # differ by 1 and both inside
                ...
                c_seen.extend(btw_c)
        if diff_P != 0:
            btw_p = _btw_chk_(p0_fr, p1_fr, p_out_c, p_seen)
            if len(btw_p) > 1:  # -- skip sequences with gaps for now eg b4,c1
                gap = btw_p[0] + len(btw_p) - 1 != btw_p[-1]
                if gap:
                    btw_p = []
            else:
                p_seen.extend(btw_p)  # B0,a has 1 value
            # if p1_fr not in [p_last, p_st_en]:  # nullify btw_p if p1_fr is
        print(f"{cnt}: {bf_c}  {bf_p}  {btw_c}  {btw_p}\n")
        #
        # -------- '0 0' prep --------------------------------------------
        if c0c1_ == '0 0':
            ...
            # sub_ is either empty or has values!
            #
            # -----------------------------------
            # -- poly checks
            # --
            ...
            # --
                ...
                if p0_fr == 0 or p1_fr > p0_fr:
                    ...
                    if btw_p:
                        sub_.extend(poly[btw_p])
                    sub_.append(x0)
                if p0_to in p_in_c:  # p0_fr not in, so check its endpoint.
            ...
            # --
                ...
                if p0_fr < p1_fr:
                    sub_.extend([x1, x0])
                    sub_.extend(poly[btw_p])  # close polygons
                else:
                    sub_.extend([x1, x0])
        ...
        # -------- '0 1' prep --------------------------------------------
            ...
            # --
                ...
                if p0_fr < p1_fr:
                    if p1_to not in btw_p:  # b4,c1  p1_to is > p0_fr & p1_fr
                        sub_.extend(poly[btw_p])
                    sub_.append(x1)
                elif p0_fr in [p_last, p_st_en]:
                    to_add = [i for i in p_out_c if i <= p1_fr]
                    sub_.extend(poly[to_add])
                    sub_.append(x1)
                # if btw_p:  # and p1_to not in btw_p:
                #     sub_.extend(poly[btw_p])
                # if p1_to in p_out_c and p1_to not in p_eq_x:  # p_seen: c2,K
                #     sub_.extend([poly[p1_to]])
                #     p_seen.extend([p1_to])
                # --
            # --
                ...
                if p0_fr == p1_fr and p0_to in c_in_p:
                    tmp = p0_to
                    sub_.extend([poly[tmp]])
                    p_seen.extend([tmp])
            # --
        ...
        # -------- '1 0' prep --------------------------------------------
            ...
            if c0_fr in c_seen and cnt != 0:  # check for last clipping line
                if c0_fr in c_eq_x and c1_to == c_st_en:
                    break
                    # last clipping line,
            if c0_fr <= c1_fr:  # added for c1, b5
                sub_.append(x0)
                if btw_c:
                    sub_.extend(clp[btw_c])
            elif c0_fr > c1_fr:
            ...
            if p0p1_ == '0 0':                             # 1,  0,  0,  0
                if p0_fr == p1_fr:  # p0_fr to p0_to (p0_to=p1_fr), eg, 2 2
        ...
        # ----
        #
        p_seen.extend([p0_fr, p1_fr, frum])
        c_seen.extend([c0_fr, c1_fr])
        sub_ = np.array(sub_)  # make sure it is an array
        # -- Assemble for the output
        # print("ply, clp seen\n{}\n{}".format(p_seen, c_seen))
        _out_ = _to_add_(xC, sub_, p_seen, c_seen, tot_)
        p_seen, c_seen, tot_ = _out_
    # --
    new_ = []
    for i in tot_:
        sub = np.asarray([j for j in i[1] if len(j) > 0])
        if not (sub[0] == sub[-1]).all(-1):
            sub = np.concatenate((sub, sub[0, None]), axis=0)
        new_.append(sub)
    new_ = np.asarray(new_, dtype='O')
