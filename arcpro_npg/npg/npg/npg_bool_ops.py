...
__all__ = ['overlay_ops']
__helpers__ = ['_cut_pairs_']
__imports__ = ['add_intersections', '_del_seq_pnts_']


# ---- (1) Operations for polygon overlays.
#
def symm_diff(poly, clp, as_geo=True):
    """Return the symmetrical difference from overlap_ops.

    Parameters
    ----------
    poly, clp : array_like
        `poly` is the polygon being overlain by polygon `clp`
    as_geo : boolean
        True, returns a Geo array. False, an object array.
    """
    result = overlay_ops()
    erase_ply, clp_ply, clp_ply2, hole_ply, symm_ply, rev_erase = result
    if as_geo:
        return npg.arrays_to_Geo(symm_ply, kind=2, info=None, to_origin=False)
    return symm_ply
