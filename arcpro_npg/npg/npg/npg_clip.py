...
def _wn_clip_(pnts, poly, all_info=True):
    """Return points in or on polygon using `winding number`.  See `inouton`.
    ...
    def _xsect_(a_num, b_num, denom, x1_x0, y1_y0, x0, y0):
        ...
        with np.errstate(all="ignore"):  # ignore all errors
            ...
            both = (z0 & z1)  # try truncating the last column
...
def split_seq(seq):
    ...
    if whr.size > 0:
        return np.array_split(seq, whr + 1)
    return seq
...
def clip(poly, clp):
    ...
    for cnt, xC in enumerate(xChk):  # xChk is the crossings list
        ...
        if cnt == 0:
            # -- before_c `bf_c`
            if c0_fr in c_in_p or diff_C == 0:
            ...
            # -- before_p `bf_p`
            if p0_fr in p_in_c or diff_P == 0:
                bf_p = _f0f1_(p0_fr, p1_fr, p_last, p_st_en,
                              p_in_c, p_eq_x, p_seen)
            if bf_p:
                p_seen.extend(bf_p)
                if p_st_en in bf_p or p_last in bf_c:
                    p_seen.extend([0])
        ...
        if diff_P != 0:
            ...
            if len(btw_p) > 1:  # -- skip sequences with gaps for now
        ...
        if c0c1_ == '0 0':
            ...
            # --
                ...
                if p0_fr == 0 or p1_fr > p0_fr:
                    sub_.append(x1)
                    if btw_p:  # -- set to [] in c0c1_ section
            ...
            # --
                ...
                if p0_fr < p1_fr:
                    ...
                    sub_.extend([x1, x0])
                if p0_to in p_in_c:  # b4,c1 pre, post points
                    sub_.extend([poly[p0_to]])
                    p_seen.extend([p0_to])
            ...
            # --
                ...
                if p0_fr < p1_fr:
                    ...
                    if btw_p and p1_to not in btw_p:
                        sub_.extend(poly[btw_p])
                if p1_to in p_in_c and p1_to not in p_eq_x:  # p_seen: c2,K
                    sub_.extend([poly[p1_to]])
                    p_seen.extend([p1_to])
                # --
            # --
