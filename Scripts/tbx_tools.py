...
def pick_tool(tool, in_fc, out_fc, gdb, name):
    ...
    # ---- Alter geometry
        ...
        dens_dist(in_fc, gdb, name, dist)
    elif tool == 'Fill Holes':                   # ---- (3) fill holes
        fill_holes(in_fc, gdb, name)
    elif tool == 'Rotate Features':              # ---- (4) rotate
        clockwise = False
        as_group = False
...
def _testing_():
    """Run in spyder."""
    in_fc = "C:/Git_Dan/npgeom/Project_npg/npgeom.gdb/Polygons"
    out_fc = "C:/Git_Dan/npgeom/Project_npg/npgeom.gdb/x"
#    tbx = pth + "/npGeom.tbx"
#    tbx = ImportToolbox(tbx)
