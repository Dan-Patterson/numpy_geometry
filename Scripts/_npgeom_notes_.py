...
%timeit get_SR(in_fc)
23 ms ± 2.29 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)

**Convert large fc to Geo array using SearchCursor**
    %timeit fc_nparray_Geo(in_fc, geom_kind=2, info="")
    6.79 s ± 146 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)

Using the __geo_interface__ to Geo array
    %timeit fc_gi_Geo(in_fc)
    19.5 s ± 433 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)

Arrays for the polygons of the above
    arrs = poly2array(z[3])
    len(arrs)  #  589
    %timeit poly2array(z[3])
    18.3 s ± 434 ms per loop (mean ± std. dev. of 7 runs, 1 loop each)
...
3.6.9 |Anaconda, Inc.| (default, Jul 30 2019, 14:00:49)
...
Geo methods and properties
    ...
    densify_by_percent, extent_centers, extent_pnts, extent_rectangles,
    extents, fill_holes, find_dupl_pnts, first_bit, first_part,
    geom_check, get_shape, hlp, holes_to_shape, info, inner_rings,
    is_clockwise, is_convex, is_in, is_multipart, lengths, maxs, means,
    min_area_rect, mins, moveto, multipart_to_singlepart, od_pairs,
    original_arrays, outer_rings, part_IFT, part_ids, parts, pnt_ids,
    pnt_on_poly, point_indices, point_info, polygons_to_polylines,
    polylines_to_polygons, polys_to_points, polys_to_segments,
    pull_shapes, radial_sort, rotate, shapes, shift, shp_IFT, shp_ids,
    shp_part_cnt, shp_pnt_cnt, sort_by_area, sort_by_extent,
    sort_by_length, sort_coords, split_by, structure, translate,
    triangulate, unique_segments, xy_id
...
in_fc = r"C:/Git_Dan/npgeom/Project_npg/npgeom.gdb/Ontario_LCConic"
SR = get_SR(in_fc)
...
# ---- intersection notes
"""
ps = np.array([[1,1], [10, 10], [0, 4], [4, 0]])
p0, p1, p2, p3 = ps
intersects(p0, p1, p2, p3)  # ---- (2, 2)
...
s_numer       # -36
t_numer       # -8
t             # 0.111111111....  1/9 the difference in the dx, dy
s_numer/denom # 0.5
t * (x1 - x0) # 1.0 where (x1 - x0) = 9
t * (y1 - y0) # 1.0 where (y1 - y0) = 9
...
a0 = p_0.__geo_interface__
a1 = ast.literal_eval(p_0.JSON)

%timeit p_0.__geo_interface__
    736 µs ± 55 µs per loop (mean ± std. dev. of 7 runs, 1000 loops each)

%timeit ast.literal_eval(p_0.JSON)
    361 µs ± 2.58 µs per loop (mean ± std. dev. of 7 runs, 1000 loops each)

%timeit [[(j.X, j.Y) for j in i if j is not None] for i in p_0]
706 µs ± 41.4 µs per loop (mean ± std. dev. of 7 runs, 1000 loops each)
