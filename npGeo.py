...
import npg_helpers as hlp
#from npg_helpers import (_angles_, _area_centroid_, _area_part_, _ch_,
#            _ch_scipy_, _ch_simple_, _nan_split_, _o_ring_, _pnts_on_line_,
...
class Geo(np.ndarray):
    ...
    @property
    def shapes(self):
        """Subdivide the array singlepart or multipart shapes as either an
    ...
    def areas(self):
        ...
        if self.K != 2:
            ...
            return None
        subs = [hlp._area_part_(i) for i in self.parts]   # call to _area_part_
        ids = self.IDs
    ...
    def centroids(self):
        ...
        for ID in uni:
            ...
            for prt in out:
                area, cen = hlp._area_centroid_(prt)  # ---- determine both
                centr.append(cen)
                areas.append(area)
    ...
    def outer_rings(self, asGeo=False):
        ...
        for ift in self.IFT:
            i, f, t = ift
            ar = self[f:t]
            p = hlp._o_ring_(ar)           # ---- call ``_o_ring_``
    ...
    def is_clockwise(self, is_closed_polyline=False):
        ...
        if self.K == 1:
            if not is_closed_polyline:
                ...
                return None
        ids = self.bit_ids
        cw = np.asarray([1 if hlp._area_part_(i) > 0. else 0
                         for i in self.bits])
    ...
    def polygon_angles(self, inside=True, in_deg=True):
        ...
        return [hlp._angles_(p, inside, in_deg) for p in self.parts]
    ...
    def convex_hulls(self, by_part=False, threshold=50):
        ...
        ch_out = [hlp._ch_(s, threshold) for s in shps]
    ...
    def triangulate(self, as_polygon=True):
        ...
        out = [hlp._tri_pnts_(s) for s in self.shapes]
        kind = 1
        if as_polygon:
            kind = 2
    ...
    def densify_by_distance(self, spacing=1):
        ...
        """
        polys = [hlp._pnts_on_line_(a, spacing) for a in self.bits]
    ...
    def pnt_on_poly(self, by_dist=True, val=1):
        ...
        """
        if by_dist:
            r = [hlp._dist_along_(a, dist=val) for a in self.bits]
            return Update_Geo(r, K=0)
        r = [hlp._percent_along_(a, percent=val) for a in self.bits]
        return Update_Geo(r, K=0)
